...
def createUser():
    ...
    if not username_ or not email_ or not password_:
        return jsonify(get_response(status_code=409, message="username / email_ / password_ is not in request")), 409
    else:
        with sqlite3.connect("UsersMicroservice.db") as con:
            cur = con.cursor()
            cur.execute("INSERT OR IGNORE INTO Users (username, email, pass) VALUES (?,?,?)", (username_, email_, password_))
            con.commit()
...
def authenticateUser():
    ...
    if not username or not password:
        return jsonify(get_response(status_code=409, message="username / password_ is not in request")), 409
    else:
        user = query_db('select * from users where username = ?', [username], one=True)
        userEnteredPasswordHash = hsh(password)
        dbPasswordHash = hsh(user[2])
        if userEnteredPasswordHash == dbPasswordHash:
